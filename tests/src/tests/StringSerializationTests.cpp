...
template< typename T>
void testComplexType( co::RefPtr<ca::IStringSerializer> serializer, T& complexType )
{
...
template< typename T>
void testArrayToAndFromString(co::RefPtr<ca::IStringSerializer>& serializer, T* staticArray, int size)
{
	co::Any input, result;
	std::string strOutput;
...
TEST( StringSerializationTests, errorSituations )
{
	...
	co::RefVector<ca::IStringSerializer> refVec;
	refVec.push_back(serializer);

	anyService.setArray(co::Any::AK_RefVector, co::typeOf<ca::IStringSerializer>::get(), 0, &refVec);
	EXPECT_THROW ( serializer->toString(anyService, actual), co::IllegalArgumentException );
