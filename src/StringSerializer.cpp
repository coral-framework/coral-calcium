...
namespace ca {
	...
	void StringSerializer::fromStream( stringstream& ss, co::IType* type, co::Any& value )
	{
	...
	void StringSerializer::readComplexType( stringstream& ss, co::Any& value, co::IType* type )
	{
	...
	template< typename T>
	T StringSerializer::readPrimitive(stringstream& ss, co::TypeKind tk)
	{
		int byte;
		T resultT;
		switch(tk)
	...
	bool StringSerializer::readBoolean( stringstream& ss )
	{
		std::string boolStr;
	...
	void StringSerializer::readComplexTypeArrayFromStream( stringstream& ss, co::IType* elementType, co::Any& value )
	{
	...
	void StringSerializer::assertNotFail( stringstream& ss, std::string additionalInfo )
	{
		if(ss.fail())
		{
			stringstream msg;
	...
	void StringSerializer::readEnumArrayFromStream(stringstream& ss, co::IType* type, co::Any& value)
	{
	...
	co::int32 StringSerializer::readEnum( stringstream& ss, co::IEnum* enumType )
	{
	...
	std::string StringSerializer::readLiteralFromStream(stringstream& ss)
	{
		std::string result;
		char buffer;
	...
	std::string StringSerializer::extractStringValueWithoutQuotes( stringstream& ss )
	{
		if(ss.peek() == '\'')
		{
	...
	std::string StringSerializer::extractQuotedString(stringstream& ss)
	{
		char readBuffer;
		std::string result;
		ss.get(readBuffer);
	...
	std::string StringSerializer::extractLongBracketsString(stringstream& ss)
	{
		char readBuffer;
		char openBrackets[4];
		std::string result;
	...
	void StringSerializer::toStream( const co::Any& value, stringstream& ss )
	{
	...
	void StringSerializer::writeEnum(const co::Any& value, stringstream& ss, co::IType* type)
	{
		co::IEnum* enumType = static_cast<co::IEnum*>(type);
	...
	void StringSerializer::writeComplexType(const co::Any& value, stringstream& ss, co::IType* type)
	{
	...
	void StringSerializer::writeArray(const co::Any& value, stringstream& ss, co::IType* type)
	{
		AnyArrayUtil arrayUtil;
	...
	void StringSerializer::writeBasicType(const co::Any& value, stringstream& ss)
	{
		co::TypeKind kind = value.getKind();
		char strNumber[32];
		switch(kind)
	...
	void StringSerializer::escapeLuaString(const std::string&  str, stringstream& ss)
	{
