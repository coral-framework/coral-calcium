...
local idCache, conversionCache, valueMap
...
local function restoreObject( spaceStore, objModel, objectId, revision )
	...
	local allValues = valueMap[ objectId ]
	...
	for k, v in pairs( allValues ) do
		if k ~= "_type" and k ~= "_provider" then
		...
		for i, value in ipairs( values ) do
			...
			if refKind == 1 then
				...
				if idServiceFieldValue == nil then
					...
					restoreService( spaceStore, objModel, tonumber(valueMap[idServiceFieldValue]._provider), idServiceFieldValue, revision )
			...
			elseif refKind == 2 then
				...
				for i, idService in ipairs( idServiceList ) do
					restoreService( spaceStore, objModel, tonumber(valueMap[idService]._provider), idService, revision )
	...
	for i, field in ipairs( fields ) do
		...
		if fieldValue ~= nil then
			...
			elseif fieldKind == 'TK_STRUCT' or fieldKind == 'TK_NATIVECLASS' then
				...
				if fieldKind == 'TK_ARRAY' and (field.type.elementType.kind == 'TK_NATIVECLASS' or field.type.elementType.kind == 'TK_STRUCT' ) then
					local fieldValuesComplex = {}
					local complexType = field.type.elementType
					for i, value in ipairs( fieldValue ) do
						fieldValuesComplex[ #fieldValuesComplex + 1 ] = coNew( complexType.fullName )
						restoreComplexType( fieldValuesComplex[ #fieldValuesComplex ], complexType, value, objModel )
					end
					service[fieldName] = fieldValuesComplex
				else
...
local function restoreFast( space, spaceStore, objModel, revision, spaceLoader )
	idCache = {}
	conversionCache = {}
	...
	local ids, fieldNames, values = spaceStore:getAllValues( revision )

	if #ids == 0 then
		error( "no values" )
	end
	valueMap = {}
	for it, id in ipairs( ids ) do
		if valueMap[id] == nil then
			valueMap[id] = {}
		end
		valueMap[id][fieldNames[it]] = values[it]
	end
